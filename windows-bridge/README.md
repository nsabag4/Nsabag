...
| `share-lan.ps1` | Windows (admin, optional) | Exposes the scan web UI/eSCL to the office LAN: `netsh` portproxy (0.0.0.0 -> 127.0.0.1, which reaches the WSL2 localhost relay) plus firewall rules, Domain/Private profiles only. Default is port 8090 (AirSane); pass `-Ports 8090,8080` to also expose scanservjs. Other PCs then scan at `http://<host>:8090` or add it in NAPS2 via Manual IP. `-Disable` reverts everything; supports `-WhatIf`. Guide: `../docs/SHARE-LAN.he.md`. |
