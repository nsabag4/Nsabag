...
**הדרך הקלה ביותר — לחיצה כפולה:** חברו את הסורק (חשמל + USB ישיר), הורידו את הקובץ `INSTALL.cmd` מהמאגר, לחצו עליו לחיצה כפולה ואשרו את בקשת ההרשאות. הוא מוריד, מחלץ ומריץ את ההתקנה לבד. (אם Windows SmartScreen מזהיר — לחצו "מידע נוסף" ← "הפעל בכל זאת".)
